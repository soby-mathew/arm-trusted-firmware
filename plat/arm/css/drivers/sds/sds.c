...
static int get_struct_info(uint32_t structure_id, struct_header_t **header)
{
	unsigned int i, structure_count;
...
int sds_struct_exists(unsigned int structure_id)
{
	...
	ret = get_struct_info(structure_id, &header);
	if (ret == SDS_OK) {
		assert(header);
		return SDS_OK;
	}
		...
		void *data, size_t size, sds_access_mode_t mode)
{
	...
	status = get_struct_info(structure_id, &header);
	if (status != SDS_OK)
		return status;
